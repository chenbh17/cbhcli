...
### 🚀 Agent上下文大幅瘦身
- **移除冗余工具描述**：tools.md 和 usage.md 精简掉冗长的逐个工具参数说明，改为统一引用 Function Calling 协议
- **系统提示优化**：`build_system_prompt()` 不再传递 `tool_descriptions`，大幅减小每次请求的系统提示大小
- **总计减少约 720+ 行**冗余内容，显著降低Token消耗和上下文占用

### 🎯 tools命令增强
- **范围选择支持**：`/tools on` 和 `/tools off` 支持输入范围语法（如 `1-5`、`1-3,7,9-12`）
- **交互更友好**：提示信息明确标注支持范围选择

### 🧹 精简工具文档
- tools.md 和 usage.md 从逐工具列举改为**核心工作流程 + Function Calling 协议引用**模式
- 工具的具体参数定义完全由 API Schema 动态提供，避免文档与实现不同步
...
### 从Wheel安装
```bash
pip install dist/cbhcli-4.8.2-py3-none-any.whl
```
